...
if __name__ == '__main__':
    ...
    set_size = 50
    n_set = 10
    network_save_count = 500         # save the checkpoint at # th episode
    results_save_count = 500         # save rewards and steps at # th episodes
    ...
    if finished_episodes != 0:
        main_network.load_state_dict(torch.load(f'results/checkpoints/pong_checkpoint' + str(
            int(finished_episodes)) + f'.pth'))
        print(f'load save network at results/checkpoints/pong_checkpoint' + str(
            int(finished_episodes)) + f'.pth')
    ...
    for t in range(total_num_steps):
        ...
        if done:
            ...
            if (((episode + 1) % network_save_count) == 0) & (episode != 0):
                counter += 1
                torch.save(main_network.state_dict(), f'results/checkpoints/pong_checkpoint' + str(
            ...
            qvalue_evert_ep.append(torch.mean(main_q_values))
            state_stack = env.reset()
        ...
        # ------------------reshape action reward done batch 32*1
        action_batch_dev = torch.reshape(action_batch_dev,(32,1))
        reward_batch_dev = torch.reshape(reward_batch_dev,(32,1))
        done_batch_dev = torch.reshape(done_batch_dev,(32,1))
